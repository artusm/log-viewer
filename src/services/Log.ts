...
export function copyToClipboardButton(v: any, copyBag: any[]): string {
  copyBag.push(v)
  return `<a><span class="icon context-button copy-button tooltip is-tooltip-right" data-tooltip="Скопировать значение в буфер обмена" data-copy="${copyBag.length - 1}"><i class="mdi mdi-content-copy"></i></span></a>`
}

export function linkToClipboardButton(cursor: any, copyBag: any[]): string {
  copyBag.push(cursor)
  return `<a><span class="icon context-button link-button tooltip is-tooltip-right" data-tooltip="Скопировать ссылку в буфер обмена" data-copy="${copyBag.length - 1}"><i class="mdi mdi-link"></i></span></a>`
}
  ...
  (function recurse(o, current, currentNonIndexKey) {
    for (const key of Object.keys(o)) {
      ...
      if (value && typeof value === "object") {
        recurse(value, newKey, newNonIndexKey)
      } else {
        res[newKey] = {
          value,
