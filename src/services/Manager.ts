...
export class Manager {
  ...
  constructor(config: Config) {
    ...
    this.config = Manager.cleanConfig(this.config)
  }
  ...
  dsFieldsConfig(): FieldsConfig {
    ...
    if (!dsConfig) {
      throw new Error('Ошибка в dsConfig')
    }

    const fieldsConfig = this.config.fields[dsConfig.fields]
    if (!fieldsConfig) {
      throw new Error(`dataSource.field - не корректное значение: dataSource.fields=${dsConfig.fields}`)
    }
  ...
  error(error) {
    ...
    if (error !== null) {
      ...
      message = "Получен null вместо ошибки."
    }
